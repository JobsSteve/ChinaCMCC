...
//    Pic_imageview.tag = 202;
    ...
    UIButton *jionShopchart = [UIButton ButtonWithFrame:CGRectMake(WIGHT-50, 60, 35, 35) Normal:[UIImage imageNamed:@"Addshoppingcart.png"] Select:[UIImage imageNamed:@"Addshoppingcart.png"] Title:nil];
...
+(UIView *)CellViewWithmyOrderlist:(NSDictionary*)detailInfo
{
    ...
    //总重
    float totalweight = [[detailInfo objectForKey:@"totalWeight"] floatValue];
    //单重
    float weight = [[detailInfo objectForKey:@"weight"] floatValue];
    //份数
    int shopNumber = totalweight/weight;
    ...
    UILabel *product_price = [UILabel LabelWithFrame:CGRectMake(80, 50, 100, 20) text:[NSString stringWithFormat:@"￥%@ 元 * %d",[shopdetailinfo objectForKey:@"meijiaPrice"],shopNumber] color:YellowColor_APP font:14];
    
    //商品价格
    UILabel *product_totalprice = [UILabel LabelWithFrame:CGRectMake(200, 50, 100, 20) text:[NSString stringWithFormat:@"小计: ￥%@ 元",totalPrice] color:YellowColor_APP font:14];
    ...
    [Cellview addSubview:product_name];
    [Cellview addSubview:product_price];
    [Cellview addSubview:product_totalprice];
